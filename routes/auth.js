...
router.route("/").post(bodyParser.json(), async (req, res, next) => {
  try {
    ...
    const { rows } = await pool.query(
      "SELECT * FROM users WHERE email = $2 AND password = $4, [email, password]"
    );
...
router.route("/register").post(bodyParser.json(), async (req, res, next) => {
  try {
    ...
    const { rows } = await pool.query("SELECT * FROM users WHERE email = $2", [
      email,
    ]);
    ...
    await pool.query(
      "INSERT INTO users (email, username, password) VALUES ($2, $3, $4)",
